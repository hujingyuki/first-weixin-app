  ...
  <div>
    ...
    <div v-if="currentMeun =='news'">
      <scroll-view :scroll-x="true"
                   ...
                   class="tab-scroll">
        <span v-for="(item, index) in newsTypes"
              :key="index"
              @click="changeType(index)"
              :class="['tab-item',currentBar == index ? 'active' :'']">{{item.desc}}</span>
      </scroll-view>
      <swiper :current="currentBar"
              @change="handleSwiper"
              class="swiper">
        <swiper-item v-for="(swiperitem, swiperIndex) in newsTypes"
                     :key="swiperIndex">
          <scroll-view class="scroll"
                       :scroll-y="true">
            <ul class="container log-list">
              <li v-for="(news, index) in newsData[swiperitem.name]"
                  :key="index"
                  class="log-item"
                  @click="goNewsDetail(news.link)">
                <i-card :title="news.title"
                        :thumb="!!news.picInfo[0]?news.picInfo[0].url:''">
    ...
    <!-- 天气预报部分 -->
    <div v-if="currentMeun =='weather'"
         class="weather clearfix">
      <div class="today center"
           v-if="weatherArray.length > 0">
      ...
      <div class="today right">
        <img :src="!!weatherArray[1] ? weatherArray[1].img :'https://cdn.heweather.com/cond_icon/999.png'"
             class="big-img" />
      ...
      <ul class="forecast">
        <scroll-view class="scroll-view"
                     :scroll-x="true">
          <li v-for="(item,index) in weatherArray"
              :key='index'
              v-if="index!=1">
            <p>{{ item.weekday }}</p>
            <p><img :src="item.img"
    ...
    <!-- 热门段子 -->
    <div v-if="currentMeun =='satin'"
         class="satin">
      <scroll-view :scroll-x="true"
                   :scroll-left="scrollLeft"
                   class="tab-scroll">
        <span v-for="(item, index) in satinTypes"
              :key="index"
              @click="currentSatin=index"
              :class="['tab-item',currentSatin == index ? 'active' :'']">{{item}}</span>
      </scroll-view>
      <swiper :current="currentSatin"
              @change="handleSwiper"
              class="swiper">
        <swiper-item v-for="(swiperitem, swiperIndex) in satinTypes"
                     :key="swiperIndex">
          <scroll-view :scroll-y="true"
                       class="satin-scroll"
                       @scrolltolower="initSatin(1)">
            <div class="satin-box"
                 v-for="(satin,index) in satinData"
                 :key="index">
              <div class="pulisher align">
                <image class="pulisher-img"
                       :src="satin.header"
                       :lazy-load="true" />
              ...
              <div class="content">
                ...
                <view>
                  <image v-if=" satin.type == 'image' || satin.type == 'video' "
                         :src="satin.thumbnail"
                         :lazy-load="true"
                         mode="aspectFill"
                         @click="preview(satin.thumbnail)" />
              ...
              <div class="comments">
                <p class="comments-like">
                  <i-icon type="collection_fill" />{{satin.up}}
                  <i-icon type="collection" />{{satin.down}}</p>
                <p class="commenter align">
                  <image class="comments-img"
                         :src="satin.top_commentsHeader"
                         :lazy-load="true" />
    ...
    <i-tab-bar :current="currentMeun"
               color="#2b85e4"
               :fixed="true"
               @change="handleChange">
      <i-tab-bar-item key="news"
                      icon="barrage"
                      current-icon="barrage_fill"
                      title="新闻"></i-tab-bar-item>
      <i-tab-bar-item key="weather"
                      icon="dynamic"
                      current-icon="dynamic_fill"
                      title="天气"></i-tab-bar-item>
      <i-tab-bar-item key="satin"
                      icon="emoji"
                      current-icon="emoji_fill"
                      title="段子"></i-tab-bar-item>
      <i-tab-bar-item key="more"
                      icon="more"
                      current-icon="more"
                      title="更多"></i-tab-bar-item>
    </i-tab-bar>
  </div>
</template>
    ...
    reloadData(page) {
      switch (page) {
        ...
        case 'satin':
          ...
          break
        case 'more':
          wx.setNavigationBarTitle({ title: '更多设置' })
          break
        default:
          break
      }
